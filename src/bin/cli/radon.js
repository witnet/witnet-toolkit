...
function traceWitnetRadonRequest (request, options) {
  ...
  if (options?.json) {
    ...
    request.sources.forEach((source, sourceIndex) => {
      const authority = source.authority?.toUpperCase().split(".").slice(-2).join(".") || (source.method === Witnet.Radon.retrievals.Methods.RNG ? "WIT/RNG" : "")
...
async function traceWitnetRadonRequestDryRun (request, options, settings) {
  ...
  let result = report?.aggregate.result
