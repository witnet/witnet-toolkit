...
import {
	...
	type GetDataRequestEtherealReport,
	type GetDataRequestFullReport,
	type GetDataRequestMode,
	type Mempool,
	Methods,
	type PeerAddr,
	type Priorities,
	type ProtocolInfo,
	type QueryStakes,
	type QueryStakingPowers,
	type SignalingInfo,
	type StakeEntry,
	type StakingPower,
	type SuperblockReport,
	type SupplyInfo,
	type SyncStatus,
...
export interface IJsonRpcProvider {
	...
	getUtxos(
		...
		filter?: { minValue?: bigint; signer?: PublicKeyHashString },
...
export class JsonRpcProvider implements IJsonRpcProvider {
	...
	public get networkId(): number | undefined {
		if (this._constants) {
			...
			obj.bootstrapHash = {
				SHA256: Array.from(utils.fromHexString(this._constants.bootstrap_hash)),
			};
	...
	public async witnesses(_since?: number): Promise<number> {
		...
		let census = 0;
		return this.stakes({ params: { distinct: true } }) // todo: implement `count` flag on IJsonRpcProvider.stakes()
			.then((records) => {
	...
	public async getSuperblock(epoch: Epoch): Promise<SuperblockReport> {
		return this.callApiMethod<SuperblockReport>(Methods.GetSuperblock, {
			block_epoch: epoch,
		});
	}
	...
	public async getUtxos(
		pkh: PublicKeyHashString,
		filter?: { minValue?: bigint | string; fromSigner?: PublicKeyHashString },
	): Promise<Array<UtxoMetadata>> {
		if (filter) {
	...
	public async searchDataRequests(
		...
		return this.callApiMethod<Array<any[]>>(Methods.SearchDataRequests, {
			radHash,
