...
import type { Opcodes as RadonReducerOpcodes } from "./reducers.js";
...
export class RadonOperator extends RadonClass {
	...
	public toString(left = "", indent = 4, level = 0): string {
		...
		let methodName = RadonOperators[this.opcode]
			.split(/(?=[A-Z])/)
			.slice(1)
			.join("");
...
export class RadonScript {
	...
	constructor(output: RadonAny) {
		...
		const inputType = Object.entries(radonTypes).find(
