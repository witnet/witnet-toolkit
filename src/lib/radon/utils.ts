...
import { fromHexString, isHexString, searchWorkspace } from "../../bin/helpers.js";
...
export function flattenRadonAssets<T extends RadonRequest | RadonTemplate | RadonModal | RadonRetrieval>(
	...
	for (const key in tree) {
		if (
			...
			entries.push(...flattenRadonAssets(tree[key], type, [...headers, key]));
		}
	}
	return entries;
}
...
function loadModuleAssets<T extends RadonRequest | RadonTemplate | RadonModal | RadonRetrieval>(options: {
	...
	const stuff = fs.existsSync(`${WITNET_ASSETS_PATH}/index.cjs`) ? require(`${WITNET_ASSETS_PATH}/index.cjs`) : {};
...
function parseScriptOperator(script: any, opcode: any, args?: any): any {
	if (!script) {
		const found = Object.entries({
			...
			"60": RadonMap,
			"70": RadonString,
		}).find((entry) => entry[0] === (parseInt(opcode, 10) & 0xf0).toString(16));
		if (found) {
			script = new found[1]();
		} else {
	...
	if (opcode) {
		let operator = RadonOperators[opcode]
			.split(/(?=[A-Z])/)
			.slice(1)
			.join("");
